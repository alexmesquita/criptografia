...
vector<int>
Des::initial_permutation(vector<int> temp) {
...
vector<int>
Des::inverse_initial_permutation(vector<int> temp) {
...
vector<int>
Des::expansion_permutation(vector<int> temp) {
...
vector<int>
Des::permutation(vector<int> temp) {
...
vector<vector<int>>
Des::convert_text(string plaintext) {
...
vector<int>
Des::left_shift(vector<int> temp, int round) {
...
vector<int>
Des::right_shift(vector<int> temp, int round) {
...
vector<int>
Des::decipher_initial_shift(vector<int> temp) {
    for (int i = 0; i < 28; ++i)
    {
        int last = temp[27];
        for(int j = 27; j > 0; j--) {
...
vector<int>
Des::permutation_choice1(vector<int> bits_key) {
...
vector<int>
Des::permutation_choice2(vector<int> ci, vector<int> di) {
...
vector<int>
Des::xor_operation(vector<int> temp1, vector<int> temp2) {
    int size = temp1.size();
...
string
Des::cipher(string plaintext, string key) {
    ...
    for (int i = 0; i < size; ++i) {
        ...
        for (int j = 0; j < 64; ++j)
        {
            cipher_bits += (result[j] + '0');
        }
    }

    string cipher_text = "";

    size = cipher_bits.size();
    stringstream ss;
    for (int i = 0; i < size; i+=4)
    {
        string temp = cipher_bits.substr(i, 4);
...
string
Des::decipher(string cipher_text, string key) {
    vector<vector<int>> bits = convert_text(cipher_text);

    vector<int> bits_key = convert_key(key);

    string decipher_bits;
    ...
    for (int i = 0; i < size; ++i) {
        ...
        for (int j = 0; j < 64; ++j)
        {
            decipher_bits += (result[j] + '0');
        }
    }

    string decipher_text = "";

    size = decipher_bits.size();
    stringstream ss;
    for (int i = 0; i < size; i+=4)
    {
        string temp = decipher_bits.substr(i, 4);
